...
def profile_detail(request, username):
    ...
    # If this person is an instructor we also need to the list of classes they teach for their faculty page
    if profile.is_instructor():
        # try/except rather than if/else in case instructor doesn't have an instructor profile.
        try:
...
def _get_query_val(dict, key):
    """
    Return None if a key has an empty value in the query dict.
    """
    try:
        if dict[key].strip():
            return dict[key]
